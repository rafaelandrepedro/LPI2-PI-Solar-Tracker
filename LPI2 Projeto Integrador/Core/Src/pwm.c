...
void PWMDutyCycle(PWM_Bus bus, int8_t dutyCycle){
	
	
	if(dutyCycle<-100||dutyCycle>100)		//Modo Pos/Neg
		return;
	
	if(bus.gpio_Port.GPIOx==0&&bus.gpio_Port.GPIO_Pin==0)		//Modo Pos
		if(dutyCycle<0)
			return;
	...
	if(bus.gpio_Port.GPIOx!=0||bus.gpio_Port.GPIO_Pin!=0){
